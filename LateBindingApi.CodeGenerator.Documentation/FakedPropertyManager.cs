...
namespace LateBindingApi.CodeGenerator.Documentation
{
    ...
    {
        DocuGenerator _parent;
        XDocument _document;
        XDocument _derived;

        internal CustomPropertyManager(DocuGenerator parent, XDocument document)
        {
            _parent = parent;
            _document = document;
        }

        public void ScanForOptionalProperties()
        {
        ...
        public bool IsDerivedReturnValue(XElement returnValue)
        {
            ...
            {
                XElement interfaceNode = DocuGenerator.GetInterfaceOrClassFromKey(typeKey);
                string id = interfaceNode.Attribute("Key").Value;
                return IsDerived(id);
            }
        }
